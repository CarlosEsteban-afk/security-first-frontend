        ...
        <div class="form-group">
          ...
          <div class="verification-code-input">
            <input type="text" v-model="verificationCode" class="form-input" required
              placeholder="Ingresa el código de 6 dígitos" maxlength="6" pattern="[0-9]{6}" />
          </div>
        </div>
...
const email = ref('') // Esto vendría de la página de registro
...
const verifyEmail = async () => {
  ...
  if (!email.value) {
    errorMessage.value = 'No se encontró el correo electrónico.'
    return
  }
  if (!isFormValid.value) return

  try {
    await axios.post('http://3.235.236.228:4000/auth/confirm-signup', {
      email: email.value,
      code: verificationCode.value
    })


    successMessage.value = 'Correo verificado correctamente. Redirigiendo...'
    setTimeout(() => {
      router.push('/login')
    }, 2000)
...
const resendCode = async () => {
  ...
  try {
    await axios.post('http://3.235.236.228:4000/auth/resend-confirmation', { email: email.value })
    successMessage.value = 'Código reenviado exitosamente.'
  } catch (error) {
    errorMessage.value = error.response?.data?.message || 'No se pudo reenviar el código.'
  }
